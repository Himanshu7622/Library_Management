...
const AppLayout: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen ${currentTheme === 'dark' ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
      {/* Mobile menu overlay */}
      {mobileMenuOpen && (
      ...
      <div className={`hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-50 lg:block lg:w-64 lg:overflow-y-auto lg:bg-white lg:border-r lg:border-gray-200 ${
        currentTheme === 'dark' ? 'lg:bg-gray-800 lg:border-gray-700' : ''
      }`}>
          ...
          <nav className="mt-5 flex-1 space-y-1 px-3">
            {navigationItems.map((item) => (
              ...
              >
                ...
                {item.id === 'dashboard' && <LayoutDashboard className="mr-3 h-5 w-5" />}
                {item.id === 'books' && <Book className="mr-3 h-5 w-5" />}
                {item.id === 'members' && <Users className="mr-3 h-5 w-5" />}
                {item.id === 'transactions' && <ArrowRightLeft className="mr-3 h-5 w-5" />}
                {item.id === 'settings' && <Settings className="mr-3 h-5 w-5" />}

                {item.label}

                {item.badge && item.badge > 0 && (
                  <span className="ml-auto inline-flex items-center justify-center rounded-full bg-red-500 px-2 py-1 text-xs font-medium text-white">
                    {item.badge}
                  </span>
                )}
              </Link>
      ...
      {/* Main content */}
      <div className={`lg:pl-64 ${sidebarOpen ? '' : ''}`}>
        {/* Top header */}
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:px-6 lg:px-8">
          <div className="flex items-center space-x-4">
            ...
            <button
              type="button"
              className="hidden -m-2.5 p-2.5 text-gray-700 lg:flex dark:text-gray-300"
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              <button
                type="button"
                className="flex items-center rounded-full bg-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:bg-gray-700"
              >
